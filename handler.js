...
—◉ *إذا كنت تعتقد أن هذا خطأ ولديك دليل، فيمكنك الاتصال بمالك الروبوت لاستئناف التعليق.* —◉ *التواصل مع مالكه:* wa.me/212646102399
╚═════════════════════╝
               `.trim();
              m.reply(messageText);
  ...
  for (const nk of callUpdate) {
    if (nk.isGroup == false) {
      if (nk.status == 'offer') {
        ...
        const vcard = `BEGIN:VCARD\nVERSION:3.0\nN:;𝗗 𝗔 𝗟 𝗔;;;\nFN:BOT\nORG:NOUREDDINE\nTITLE:\nitem1.TEL;waid=212646102399:212646102399\nitem1.X-ABLabel:noureddine\nX-WA-BIZ-DESCRIPTION:[❗] contact noureddine ouafy\nEND:VCARD`;
        await mconn.conn.sendMessage(nk.from, {contacts: {displayName: '𝗗 𝗔 𝗟 𝗔', contacts: [{vcard}]}}, {quoted: callmsg});
...
global.dfail = (type, m, conn) => {
  ...
  const prep = generateWAMessageFromContent(m.chat, {extendedTextMessage: {text: msg, contextInfo: {externalAdReply: {title: '*[ ⚠ ] Warning*', body: '𝗗 𝗔 𝗟 𝗔BOT', thumbnail: imagen1, sourceUrl: 'https://https://www.instagram.com/3abdollah_dl?igsh=MTNsdTltaTRoZG92Zw=='}}}}, aa);
